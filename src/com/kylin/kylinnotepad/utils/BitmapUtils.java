...
public class BitmapUtils {
	 ...
	 * @return
	 */
	public static Bitmap decodeSampledBitmapFromFile(Resources resource,String pathName,
			int reqWidth) {
		final BitmapFactory.Options options = new BitmapFactory.Options();
		options.inJustDecodeBounds = true;
		BitmapFactory.decodeFile(pathName, options);
		int reqHeight = (int) (1.0* options.outHeight / options.outWidth * reqWidth);
		options.inSampleSize = calculateInSampleSize(options, reqWidth,
				reqHeight);
		options.inJustDecodeBounds = false;
		Bitmap src = BitmapFactory.decodeFile(pathName, options);
		if (src == null){
			return decodeSampledBitmapFromResource(resource, R.drawable.img_default, reqWidth, reqWidth);
		}
		return createScaleBitmap(src, reqWidth, reqHeight, options.inSampleSize);
	}
}